...
plane_map<Kernel> snap_halfspaces(const std::list<CGAL::Plane_3<Kernel>>& planes, double search_radius) {
	...
	for (size_t i = 0; i < planes.size(); ++i) {
		...
		auto avg = Kernel::Plane_3(sum2[0] / N, sum2[1] / N, sum2[2] / N, sum2[3] / N);
...
public:
	...
	virtual std::unique_ptr<halfspace_tree> map(const std::map<typename Kernel::Plane_3, typename Kernel::Plane_3, PlaneLess<Kernel>>& m) const {
...
void bfs(Graph<Kernel>& g, size_t start_vertex, Fn& fn) {
	...
	while (!queue.empty()) {
		...
		boost::graph_traits<Graph<Kernel>>::out_edge_iterator ei, ei_end;
...
std::unique_ptr<halfspace_tree<TreeKernel>> build_halfspace_tree(Graph<Kernel>& G, CGAL::Nef_polyhedron_3<Kernel>& poly, bool negate = false, int level=0) {
	...
	boost::graph_traits<Graph<Kernel>>::edge_iterator ei, ei_end;
	...
	*/
	...
	for (size_t i = 0; i <= *std::max_element(components_0.begin(), components_0.end()); ++i) {
		...
		auto included_in_vertex_subset_0 = [&components_0, &i](Graph<Kernel>::vertex_descriptor vd) {
		...
		auto included_in_vertex_subset = [&components, &largest_component_idx](Graph<Kernel>::vertex_descriptor vd) {
	...
	if (false && level == 0) {
		...
		auto make_vertex_point_it = [](CGAL::Nef_polyhedron_3<Kernel>::Vertex_const_iterator p) {
...
bool convert_to_polyhedron(const CGAL::Nef_polyhedron_3<Kernel>& a, CGAL::Polyhedron_3<Kernel>& b, size_t volume_index=0) {
	...
	for (auto it = a.volumes_begin(); it != a.volumes_end(); ++it) {
		...
		if (v == volume_index) {
			...
			a.visit_shell_objects(CGAL::Nef_polyhedron_3<Kernel>::SFace_const_handle(it->shells_begin()), vis);
			vis.build(b);		
			return true;
		}
		v++;
