...
plane_map<Kernel> snap_halfspaces(const std::list<CGAL::Plane_3<Kernel>>& planes, double search_radius) {
	...
	for (auto& p : planes) {
		...
		Point_d p(CGAL::to_double(p.a()) / l, CGAL::to_double(p.b()) / l, CGAL::to_double(p.c()) / l, CGAL::to_double(p.d()) / l);
...
public:
	...
	virtual CGAL::Nef_polyhedron_3<Kernel> evaluate(int level) const {
		...
		if constexpr(CGAL::Is_extended_kernel<Kernel>::value_type::value) {
			...
			/*
			// @todo
			for (auto& v : volumes(two_halfspaces)) {
				v->mark() = 0;
			}
			*/
			throw std::runtime_error("");
		} else {
			typename Kernel::Vector_3 ref;

			static int N = 1;

			if (CGAL::abs(plane_.orthogonal_direction().dz()) > CGAL::abs(plane_.orthogonal_direction().dx())) {
				ref = typename Kernel::Vector_3(1, 0, 0);
			} else {
				ref = typename Kernel::Vector_3(0, 0, 1);
			}
